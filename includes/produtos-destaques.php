            <div id="produtosDestaque" class="blocoProdutos">
                <div class="boxTituloBlocos">
                    <span class="titulo">Produtos</span>
                    <span class="subTitulo">Em Destaque</span>
                </div>

                <div id="bannerProdutosDestaques">
                    <div class="bannerDestaque">
                        <div class="destaqueMaior">
                            <a class="linkDestaques" href="javascript:;">
                                <img class="imgDestaque imgDestaqueMaior" src="img/conteudo/produto-destaque.png" alt=""/>
                                <span class="hoverDestaques"></span>
                            </a>
                            <div class="informacoesProduto">
                                <span class="titulo">Bolsa - 4174V33</span>
                                <span class="preco"><span class="textoSublinhado">De R$ 709,90</span> / Por R$ 426,90</span>

                                <div class="aPrazo">
                                    <span class="numeroVezes"><span>6x</span> s/ juros</span>
                                    <span class="valorParcela">71<span class="valorParcelaCentavos">,00</span></span>
                                </div>
                            </div>
                        </div>

                        <div class="destaqueMenor">
                            <a class="boxImagem linkDestaques" href="javascript:;">
                                <img class="imgDestaque imgDestaqueMenor" src="img/conteudo/produto-destaque-menor.png" alt=""/>
                                <span class="hoverDestaques"></span>
                            </a>
                            <div class="informacoesProduto">
                                <span class="titulo">Bolsa - 4174V33</span>
                                <span class="preco"><span class="textoSublinhado">De R$ 709,90</span> / Por R$ 426,90</span>
                                <div class="aPrazo">
                                    <span class="numeroVezes"><span>6x</span> s/ juros</span>
                                    <span class="valorParcela">71<span class="valorParcelaCentavos">,00</span></span>
                                </div>
                            </div>
                        </div>

                        <div class="destaqueMenor destaqueMenorMargem">
                            <a class="boxImagem linkDestaques" href="javascript:;">
                                <img class="imgDestaque imgDestaqueMenor" src="img/conteudo/produto-destaque-menor2.png" alt=""/>
                                <span class="hoverDestaques"></span>
                            </a>
                            <div class="informacoesProduto">
                                <span class="titulo">Bolsa - 4174V33</span>
                                <span class="preco"><span class="textoSublinhado">De R$ 709,90</span> / Por R$ 426,90</span>
                                <div class="aPrazo">
                                    <span class="numeroVezes"><span>6x</span> s/ juros</span>
                                    <span class="valorParcela">71<span class="valorParcelaCentavos">,00</span></span>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="bannerDestaque">
                        <div class="destaqueMaior">
                            <a class="linkDestaques" href="javascript:;">
                                <img class="imgDestaque imgDestaqueMaior" src="img/conteudo/produto-destaque2.png" alt=""/>
                                <span class="hoverDestaques"></span>
                            </a>
                            <div class="informacoesProduto">
                                <span class="titulo">Bolsa - 4174V33</span>
                                <span class="preco"><span class="textoSublinhado">De R$ 709,90</span> / Por R$ 426,90</span>

                                <div class="aPrazo">
                                    <span class="numeroVezes"><span>6x</span> s/ juros</span>
                                    <span class="valorParcela">71<span class="valorParcelaCentavos">,00</span></span>
                                </div>
                            </div>
                        </div>

                        <div class="destaqueMenor">
                            <a class="boxImagem linkDestaques" href="javascript:;">
                                <img class="imgDestaque imgDestaqueMenor" src="img/conteudo/produto-destaque-menor2.png" alt=""/>
                                <span class="hoverDestaques"></span>
                            </a>
                            <div class="informacoesProduto">
                                <span class="titulo">Bolsa - 4174V33</span>
                                <span class="preco"><span class="textoSublinhado">De R$ 709,90</span> / Por R$ 426,90</span>
                                <div class="aPrazo">
                                    <span class="numeroVezes"><span>6x</span> s/ juros</span>
                                    <span class="valorParcela">71<span class="valorParcelaCentavos">,00</span></span>
                                </div>
                            </div>
                        </div>

                        <div class="destaqueMenor destaqueMenorMargem">
                            <a class="boxImagem linkDestaques" href="javascript:;">
                                <img class="imgDestaque imgDestaqueMenor" src="img/conteudo/produto-destaque-menor.png" alt=""/>
                                <span class="hoverDestaques"></span>
                            </a>
                            <div class="informacoesProduto">
                                <span class="titulo">Bolsa - 4174V33</span>
                                <span class="preco"><span class="textoSublinhado">De R$ 709,90</span> / Por R$ 426,90</span>
                                <div class="aPrazo">
                                    <span class="numeroVezes"><span>6x</span> s/ juros</span>
                                    <span class="valorParcela">171<span class="valorParcelaCentavos">,00</span></span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
